...
{
    MLOGF(I, "[NVS] write size=%lu, first_byte=0x%02X\r\n", (unsigned long)size, source[0]);
    if (size > NVS_MAX_SIZE) {
        return false;
    }
    ...
    for (uint32_t i = 0; i < num_words; i++) {
        ...
        if (status != FLASH_OPERATE_DONE) {
            MLOGF(E, "[NVS] write failed at word %lu, status=%d\r\n", (unsigned long)i, (int)status);
            flash_lock();
            return false;
        }
    }
    ...
    MLOGF(I, "[NVS] write success! immediately read back first word: 0x%08X (expected: 0x%08X)\r\n", 
