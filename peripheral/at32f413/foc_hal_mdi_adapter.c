 ...
 * @brief   AT32F413 MDI to instance-scoped FOC HAL adapter
 ******************************************************************************/
                                     ...
                                     q_type qDutyW)
{
    ...
    uint32_t wU = (uint32_t)(qDutyU * ptContext->wPwmPeriod);
    uint32_t wV = (uint32_t)(qDutyV * ptContext->wPwmPeriod);
    uint32_t wW = (uint32_t)(qDutyW * ptContext->wPwmPeriod);
#else
...
#endif
    nResult = port_mdi_MotorPwmSetDuty3(wU, wV, wW);
...
{
    (void)pContext;
    /* TMR1 CH4 triggers preempt conversion; no software start is required. */
                                    ...
                                    uint32_t *pwRawW)
{
    ...
    if (pwRawU != NULL) *pwRawU = hwU;
    if (pwRawV != NULL) *pwRawV = hwV;
    if (pwRawW != NULL) *pwRawW = hwW;
    return FOC_RESULT_OK;
}
