...
| 阶段 | V0（优化前） | V1（LUT + BAM32 + cached Park/IPark） | V2.1（零开销 Profile + DWT 直读 + 指令级瘦身） |
|---|---|---|---|
| 开环总 cycles | ~7,442 (87.5%) | ~5,241 (61.7%) | **3,588 (42.2%)** |
| 电流闭环总 cycles | 未测（libm 溢出风险） | ~5,200 (61.2%) | **4,306–4,378 (51.5%)** |
| 主要手段 | libm sinf×4 + floorf | 513 项 LUT + BAM32 角度 + Park/IPark 单次计算 | 对象化 Profile、DWT 架构地址直读、Level 0 编译期零开销 |
